...
// migrations 13 (retry-buffer), 14 (nct-salt) and 15 (companion-meta-nonce); all of
// them are idempotent (CREATE TABLE/INDEX IF NOT EXISTS, ADD COLUMN IF NOT EXISTS),
// so re-applying on a fresh v15 install is a no-op.
